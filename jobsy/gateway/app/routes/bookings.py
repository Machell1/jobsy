...
from ..models_bookings import Booking, BookingEvent, Quote
...
async def reschedule_booking(
    ...
    now = datetime.now(UTC)
    booking.scheduled_date = date_type.fromisoformat(
        data.scheduled_date,
    )
...
async def create_quote(
    ...
    if booking.provider_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the provider can send quotes",
        )
...
async def respond_to_quote(
    ...
    if booking.customer_id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the customer can respond to quotes",
        )
