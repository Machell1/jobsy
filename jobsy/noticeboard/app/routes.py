"""Notice board service API routes -- boards, posts, comments, likes, moderation."""
...
from .models import Comment, NoticeBoard, Post, PostLike
...
async def create_board(
    ...
    result = await db.execute(select(NoticeBoard).where(NoticeBoard.user_id == user_id))
    existing = result.scalar_one_or_none()
    if existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Notice board already exists")
...
async def get_my_board(
    ...
    if not board:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Notice board not found")
    return _board_response(board)
...
async def create_post(
    ...
    board = result.scalar_one_or_none()
    if not board:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No notice board found. Create one first.")
...
async def list_my_posts(
    ...
    if status_filter:
        query = query.where(Post.moderation_status == status_filter)
...
async def public_feed(
    ...
    query = select(Post).where(Post.moderation_status == "approved")
    if category:
...
async def update_post(
    ...
    """Edit a post. Resets to pending_review if was approved."""
    user_id = _get_user_id(request)
...
async def list_comments(
    ...
    result = await db.execute(
        select(Comment)
        .where(Comment.post_id == post_id, Comment.moderation_status == "approved")
...
async def like_target(
    ...
    return {"status": "liked", "target_type": data.target_type, "target_id": data.target_id}
...
async def list_pending(
    item_type: str = Query(default="post", alias="type"),
    page: int = Query(default=1, ge=1),
    limit: int = Query(default=20, le=100),
    db: AsyncSession = Depends(get_db),
):
    """List pending posts and comments for moderation."""
    offset = (page - 1) * limit

    if item_type == "comment":
        result = await db.execute(
            select(Comment)
            .where(Comment.moderation_status == "pending_review")
            .order_by(Comment.created_at)
            .offset(offset)
            .limit(limit)
        )
        ...
        result = await db.execute(
            select(Post)
            .where(Post.moderation_status == "pending_review")
...
async def review_item(
    ...
    if item_type == "post":
        result = await db.execute(select(Post).where(Post.id == item_id))
        ...
        result = await db.execute(select(Comment).where(Comment.id == item_id))
        item = result.scalar_one_or_none()
        if not item:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Comment not found")
        item.moderation_status = new_status
