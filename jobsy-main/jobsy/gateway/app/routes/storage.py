"""Storage routes merged into the gateway (formerly storage microservice)."""
...
# Import S3 utilities from the storage service package
from storage.app.s3 import (
    ALLOWED_DOC_TYPES,
    ALLOWED_IMAGE_TYPES,
    ALLOWED_VIDEO_TYPES,
    create_thumbnail,
...
def _require_admin(user: dict) -> None:
    ...
    role = user.get("active_role") or user.get("role", "user")
...
async def upload(
    ...
    """Upload a file. Supports images, videos, and PDFs."""
...
async def remove_file(
    ...
    if role != "admin" and "admin" not in user.get("roles", []) and user_id not in key:
        raise HTTPException(
