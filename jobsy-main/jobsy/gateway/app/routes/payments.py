...
PLATFORM_FEE_PERCENT = Decimal(os.getenv("PLATFORM_FEE_PERCENT", "3.2"))

# Contract creation fee (flat fee in JMD charged per contract)
CONTRACT_CREATION_FEE = Decimal(os.getenv("CONTRACT_CREATION_FEE_JMD", "750"))

# Processing fee estimate (Stripe charges ~2.9% + 30c, Jamaica cards ~3.2%)
PROCESSING_FEE_PERCENT = Decimal(os.getenv("PROCESSING_FEE_PERCENT", "3.2"))
...
async def _create_intent(
    ...
    desc: str | None, meta: dict | None, transaction_id: str | None = None,
) -> dict | None:
    if not _stripe_ok():
        return None
    ...
    if transaction_id:
        params["idempotency_key"] = f"pay_{transaction_id}"
...
async def _create_escrow_intent(
    amount: Decimal, customer_id: str | None, connect_id: str | None,
    desc: str | None, meta: dict | None, transaction_id: str | None = None,
) -> dict | None:
    """Create a PaymentIntent with capture_method='manual' for escrow holds."""
    if not _stripe_ok():
        return None
    cents = int(amount * 100)
    fee_cents = int(amount * PLATFORM_FEE_PERCENT / 100 * 100)
    ...
    if connect_id:
        ...
        params["application_fee_amount"] = fee_cents
    if transaction_id:
        params["idempotency_key"] = f"escrow_{transaction_id}"
    intent = stripe.PaymentIntent.create(**params)
    return {"payment_intent_id": intent.id, "client_secret": intent.client_secret}


async def create_escrow_hold_for_contract(
    contract_id: str, amount: Decimal, payer_id: str, payee_id: str,
    db: AsyncSession,
) -> dict:
    """Create an escrow hold transaction for a contract. Used internally."""
    ...
    payee_r = await db.execute(select(PaymentAccount).where(PaymentAccount.user_id == payee_id))
    ...
    meta = {"payer_id": payer_id, "payee_id": payee_id, "contract_id": contract_id, "type": "escrow"}
    intent = await _create_escrow_intent(
        amount, customer_id, connect_id,
        f"Escrow hold for contract {contract_id}", meta,
        transaction_id=txn_id,
    )

    now = datetime.now(UTC)
    txn = Transaction(
        id=txn_id, payer_id=payer_id, payee_id=payee_id,
        contract_id=contract_id,
        amount=amount, currency="JMD", platform_fee=fee, net_amount=net,
        provider_payout=net,
        stripe_payment_intent_id=intent["payment_intent_id"] if intent else None,
        escrow_payment_intent_id=intent["payment_intent_id"] if intent else None,
        escrow_status="held",
        status="escrow_held",
        description=f"Escrow hold for contract {contract_id}",
...
async def initiate_payment(
    ...
    if amount < Decimal("100"):
        raise HTTPException(
            status_code=400,
            detail="Minimum payment amount is 100 JMD",
        )
    ...
    now = datetime.now(UTC)
    txn = Transaction(
        id=txn_id, payer_id=payer_id, payee_id=data.payee_id,
...
async def create_refund(
    ...
    # Wrap refund creation and transaction status update in a single DB
    # transaction to prevent race conditions (e.g. double-refund).
...
@router.post("/escrow/hold", status_code=201)
async def escrow_hold(
    data: EscrowHold,
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Create an escrow hold (manual-capture PaymentIntent) for a contract."""
    amount = Decimal(str(data.amount))
    if amount < Decimal("100"):
        raise HTTPException(status_code=400, detail="Minimum escrow amount is 100 JMD")

    # Check no existing escrow for this contract
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=400, detail="Escrow already held for this contract")

    result = await create_escrow_hold_for_contract(
...
async def escrow_release(
    ...
    """Capture the held PaymentIntent, releasing funds. Only callable by hirer."""
    result = await db.execute(
        select(Transaction).where(
            Transaction.contract_id == data.contract_id,
            Transaction.escrow_status == "held",
    ...
    # Only hirer can release
    if txn.payer_id != user["user_id"]:
        raise HTTPException(status_code=403, detail="Only the hirer can release escrow")

    # Check for active disputes
    from ..models_payments import Dispute
    dispute_r = await db.execute(
        select(Dispute).where(
            Dispute.contract_id == data.contract_id,
            Dispute.status.in_(["open", "responded", "under_review"]),
        )
    )
    if dispute_r.scalar_one_or_none():
        raise HTTPException(status_code=400, detail="Cannot release escrow while dispute is active")

    # Capture on Stripe
    if _stripe_ok() and txn.escrow_payment_intent_id:
        try:
            stripe.PaymentIntent.capture(txn.escrow_payment_intent_id)
...
async def escrow_refund(
    ...
    """Cancel the held PaymentIntent (refund to customer). Used during disputes."""
    result = await db.execute(
        select(Transaction).where(
            Transaction.contract_id == data.contract_id,
            Transaction.escrow_status == "held",
    ...
    if _stripe_ok() and txn.escrow_payment_intent_id:
        try:
            stripe.PaymentIntent.cancel(txn.escrow_payment_intent_id)
        except stripe.error.StripeError as e:
            raise HTTPException(status_code=400, detail=f"Stripe cancel failed: {str(e)}")

    now = datetime.now(UTC)
    txn.escrow_status = "refunded"
    txn.status = "refunded"
    txn.description = f"{txn.description or ''} | Refund reason: {data.reason or 'N/A'}"
    txn.updated_at = now
    await db.flush()
...
async def cleanup_stale_payments(
    ...
    await db.flush()
    logger.info("Expired %d stale pending transactions", len(expired_ids))
