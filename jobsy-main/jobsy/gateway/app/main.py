...
async def _apply_migrations() -> None:
    ...
    try:
        async with engine.begin() as conn:
            ...
            # ── Migration 020: Pan di Ends (Events) ──
            await conn.execute(
                text(
                    ...
                    "ticket_sold_count INTEGER DEFAULT 0, "
            ...
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_organizer ON events(organizer_id)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_category ON events(category)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_parish ON events(parish)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_start_date ON events(start_date)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_status ON events(status)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_is_free ON events(is_free)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_is_featured ON events(is_featured)"))
            ...
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_ticket_event ON event_tickets(event_id)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_ticket_user ON event_tickets(user_id)"))
            ...
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_rsvp_event ON event_rsvps(event_id)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_rsvp_user ON event_rsvps(user_id)"))
            ...
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_update_event ON event_updates(event_id)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_event_update_created ON event_updates(created_at)"))

            # ── Migration 021: Revenue engine -- Ads, boost, platform config ──
            # Extend ad_campaigns with gateway-managed columns
            await conn.execute(
                text("ALTER TABLE ad_campaigns ADD COLUMN IF NOT EXISTS advertiser_id VARCHAR")
            )
            await conn.execute(
                text("ALTER TABLE ad_campaigns ADD COLUMN IF NOT EXISTS name VARCHAR(200)")
            )
            await conn.execute(
                text("ALTER TABLE ad_campaigns ADD COLUMN IF NOT EXISTS ad_type VARCHAR(30) DEFAULT 'banner'")
            )
            await conn.execute(
                text("ALTER TABLE ad_campaigns ADD COLUMN IF NOT EXISTS creative_url VARCHAR(500)")
            )
            await conn.execute(
                text("ALTER TABLE ad_campaigns ADD COLUMN IF NOT EXISTS creative_type VARCHAR(20) DEFAULT 'image'")
            )
            await conn.execute(
                text("ALTER TABLE ad_campaigns ADD COLUMN IF NOT EXISTS budget NUMERIC(12,2)")
            )
            await conn.execute(
                text("ALTER TABLE ad_campaigns ADD COLUMN IF NOT EXISTS pricing_model VARCHAR(10) DEFAULT 'cpm'")
            )
            await conn.execute(
                text("ALTER TABLE ad_campaigns ADD COLUMN IF NOT EXISTS admin_note TEXT")
            )
            await conn.execute(
                text("CREATE INDEX IF NOT EXISTS idx_adcampaign_advertiser ON ad_campaigns(advertiser_id)")
            )

            # Ad impressions -- add placement column (separate from placement_id)
            await conn.execute(
                text("ALTER TABLE ad_impressions ADD COLUMN IF NOT EXISTS placement VARCHAR(100)")
            )

            # Ad clicks -- add placement column
            await conn.execute(
                text("ALTER TABLE ad_clicks ADD COLUMN IF NOT EXISTS placement VARCHAR(100)")
            )

            # Job post boost columns
            await conn.execute(
                text("ALTER TABLE job_posts ADD COLUMN IF NOT EXISTS is_boosted BOOLEAN DEFAULT false")
            )
            await conn.execute(
                text("ALTER TABLE job_posts ADD COLUMN IF NOT EXISTS boost_expires_at TIMESTAMPTZ")
            )
            await conn.execute(
                text("CREATE INDEX IF NOT EXISTS idx_jp_boosted ON job_posts(is_boosted)")
            )

            # Platform config table for admin fee management
            await conn.execute(
                text(
            ...
            await conn.execute(
                text("CREATE INDEX IF NOT EXISTS idx_platform_config_key ON platform_config(key)")
            )

            # Seed default fee values
            for fee_key, fee_val, fee_desc in [
                ("platform_fee_percent", "10", "Platform commission percentage"),
                ("event_posting_fee", "500", "Fee for posting an event (JMD)"),
                ("boost_fee", "1500", "Fee for boosting a job post (JMD)"),
                ("ad_cpm_rate", "50", "Default CPM rate for ads (JMD)"),
                ("ad_cpc_rate", "5", "Default CPC rate for ads (JMD)"),
            ]:
                await conn.execute(
                    text(
                        "INSERT INTO platform_config (id, key, value, description, updated_at) "
                        "VALUES (:id, :key, :val, :desc, now()) "
                        "ON CONFLICT (key) DO NOTHING"
                    ),
                    {"id": f"cfg_{fee_key}", "key": fee_key, "val": fee_val, "desc": fee_desc},
                )

            # ── Migration 022: Trust & Verification -- Escrow, Disputes, Badges ──
            # Add escrow columns to transactions
            await conn.execute(
                text("ALTER TABLE transactions ADD COLUMN IF NOT EXISTS contract_id VARCHAR")
            )
            await conn.execute(
                text("ALTER TABLE transactions ADD COLUMN IF NOT EXISTS escrow_status VARCHAR(20)")
            )
            await conn.execute(
                text("ALTER TABLE transactions ADD COLUMN IF NOT EXISTS escrow_payment_intent_id VARCHAR(100)")
            )
            await conn.execute(
                text("CREATE INDEX IF NOT EXISTS idx_txn_contract ON transactions(contract_id)")
            )
            await conn.execute(
                text("CREATE INDEX IF NOT EXISTS idx_txn_escrow ON transactions(escrow_status)")
            )
            ...
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_dispute_contract ON disputes(contract_id)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_dispute_status ON disputes(status)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_dispute_raiser ON disputes(raiser_id)"))
            ...
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_vb_user ON verification_badges(user_id)"))
...
def sanitize_input(text: str, max_length: int = 10_000) -> str:
    """Strip HTML tags, collapse whitespace, and enforce a length limit.

    Use this on user-supplied text fields before persisting or forwarding.
    """
